//
//  AndOrQueryClauseViewController.swift
//  SampleProject
//
...
protocol AndOrQueryClauseViewControllerDelegate {
    func saveClause(_ newClause: QueryClause)
}

class AndOrQueryClauseViewController: KiiBaseTableViewController, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate, AndOrQueryClauseViewControllerDelegate, StatusTableViewCellDelegate, IntervalStatusCellDelegate {

    var andOrClause: QueryClause!
    var delegate: AndOrQueryClauseViewControllerDelegate?

    fileprivate var subClauses = [QueryClause]()
    ...
    override func viewDidLoad() {
        ...
        if self.andOrClause != nil {
            if andOrClause is AndClauseInQuery {
                let andClause = andOrClause as! AndClauseInQuery
                subClauses = andClause.clauses
            }else {
                let orClause = andOrClause as! OrClauseInQuery
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.row < subClauses.count {
            if let clause = subClauses[indexPath.row] as? RangeClauseInQuery {
        ...
        if indexPath.row == subClauses.count { // Cell for Add Clause
            ...
            if clause is AndClauseInQuery || clause is OrClauseInQuery {
                ...
                case .LeftOpen, .RightOpen, .BothOpen, .BothClose:
                    upperLimitValue = (clause as! RangeClauseInQuery).upperLimit as? Int
                    lowerLimitValue = (clause as! RangeClauseInQuery).lowerLimit as? Int
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row < subClauses.count {
            ...
            if clauseSelected is AndClauseInQuery || clauseSelected is OrClauseInQuery {
                let storyBoard = UIStoryboard(name: "Triggers", bundle: nil)
                if let andOrVC = storyBoard.instantiateViewController(withIdentifier: "AndOrQueryClauseViewController") as? AndOrQueryClauseViewController {
    ...
    func showPickerView() {
        ...
        buttonCancel.addTarget(self, action: #selector(AndOrQueryClauseViewController.cancelPicker(_:)), for: UIControlEvents.touchDown)
    ...
    func selectClauseAndStatus(_ sender: UIButton) {
        if let clauseTypeSelected = clauseTypeTempSelected {
            var clauseSelected: QueryClause?
    ...
    @IBAction func tapSave(_ sender: AnyObject) {
        if self.delegate != nil {
            if andOrClause is AndClauseInQuery {
                var newClause = AndClauseInQuery()
                for subClause in subClauses {
                    newClause.add(subClause)
                }
                delegate!.saveClause(newClause)
            }else {
                var newClause = OrClauseInQuery()
                for subClause in subClauses {
                    newClause.add(subClause)
                }
                delegate!.saveClause(newClause)
    ...
    func setIntervalStatus(_ sender: UITableViewCell, lowerLimitValue: AnyObject, upperLimitValue: AnyObject) {
        let indexPath = self.tableView.indexPath(for: sender)!
        let clause = subClauses[indexPath.row] as! RangeClauseInQuery
        let status = ClauseHelper.getStatusFromClause(clause)
        if let statusSchema = schema?.getStatusSchema(status) {
